...
class FlowResistor(HeatExchangerSimple):
    r"""
    ...
    **Optional Equations**
    ...
    def get_variables(self):
        return {
            ...
            'A': dc_cp(min_val=1e-6, max_val=2, d=1e-4),
            'zet': dc_cp(min_val=0, max_val=10, d=1e-4),
            ...
            'flow_resistor_group': dc_gcp(
                elements=['A', 'zet'], num_eq=1,
                latex=self.flow_resistor_func_doc,
                func=self.flow_resistor_func, deriv=self.flow_resistor_deriv),
            'kA_group': dc_gcp(
    ...
    def flow_resistor_func(self):
        ...
        return ((i[1] - o[1]) - self.zet.val * abs(i[0]) * i[0] * (v_i + v_o) / 2 /
                (2 * self.A.val**2))

    def flow_resistor_func_doc(self, label):
        r"""
        Equation for pressure drop calculation from darcy friction factor.
    ...
    def flow_resistor_deriv(self, increment_filter, k):
        r"""
